...
thread::detail::tls_key_type key;
...
void run()
{
    at_thread_exit_elt* elt = static_cast<at_thread_exit_elt*>(thread::detail::tls_get(key));
    thread::detail::tls_set(key, SROOK_NULLPTR);
    run(elt);
}
...
{
    struct key_s {
        key_s() { thread::detail::tls_create(&key, run); }
        ~key_s() { thread::detail::tls_delete(key); }
    };
    static key_s ks;
    std::atexit(run);
}
...
{
    static thread::detail::exec_once_flag_type once = SROOK_EXEC_ONCE_INITIALIZER;
    thread::detail::execute_once(&once, key_init);

    elt->next = static_cast<at_thread_exit_elt*>(thread::detail::tls_get(key));
    thread::detail::tls_set(key, elt);
}
