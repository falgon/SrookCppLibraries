...
#define EMPTY_TYPE(N)\
	template <class T, SROOK_PP_LOOP(CLASS_DECL, N)>\
...
#define VOID_T_IMPL(N)\
	template <SROOK_PP_LOOP(CLASS_DECL, N)>\
	struct void_t_impl_##N : Empty_type_##N<void, SROOK_PP_LOOP(LOOP_CNT, N)> {};

#define VOID_T_N(N)\
	template <SROOK_PP_LOOP(CLASS_DECL, N)>\
	struct voider_##N {\
		typedef typename utility::detail::void_t_impl_##N<SROOK_PP_LOOP(LOOP_CNT, N)>::type type;\
	};
