...
constexpr bool has_iterator_v<
    ...
    std::void_t<
	typename T::iterator,
	std::enable_if_t<!has_iterator_category_v<T>>>> = std::true_type::value;

template <class T>
constexpr bool has_iterator_v<
    T,
    std::enable_if_t<has_iterator_category_v<T>>> = std::conditional<std::is_base_of<std::iterator<typename T::iterator_category, void, void, void, void>, T>::value ||
