...
#ifndef INCLUDED_SROOK_TUPLE_ALGORITHM_FOLDR1_HPP
#define INCLUDED_SROOK_TUPLE_ALGORITHM_FOLDR1_HPP
...
auto
#endif
foldr1(F&& f, const std::tuple<T, Ts...>& ts) 
SROOK_NOEXCEPT(
...
{
    return foldr(srook::forward<F>(f), std::get<0>(ts), drop<1>(ts));
}
