...
template <
    ...
    class Functor,
    REQUIRES(
	(has_iterator_v<remove_ref_cv(Range)> or is_range_iterator_v<remove_ref_cv(Range)>)and is_callable_v<remove_ref_cv(Functor)>)>
...
template <
    ...
    class Functor,
    REQUIRES(
	is_callable_v<remove_ref_cv(Functor)> and
