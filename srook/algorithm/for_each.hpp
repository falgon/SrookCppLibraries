...
template <class Iterator, class Functor, REQUIRES(!has_iterator_v<remove_ref_cv(Iterator)> and is_callable_v<remove_ref_cv(Functor)>)>
...
template <class Range, class Functor, REQUIRES((has_iterator_v<remove_ref_cv(Range)> or is_range_iterator_v<remove_ref_cv(Range)>)and is_callable_v<remove_ref_cv(Functor)>)>
...
template <class Tuple, class Functor, REQUIRES(is_callable_v<remove_ref_cv(Functor)> and !std::is_const<Tuple>::value and (std::tuple_size<std::remove_reference_t<Tuple>>::value or !std::tuple_size<std::remove_reference_t<Tuple>>::value))>
auto for_each(Tuple &&t, Functor &&functor) noexcept(noexcept(tuple_for_eacher(std::declval<decltype(static_cast<Tuple &&>(t))>(), std::declval<decltype(static_cast<Functor &&>(functor))>(), std::declval<std::make_index_sequence<std::tuple_size<std::decay_t<Tuple>>::value>>())))
...
template <class... Ts, class Functor, REQUIRES(is_callable_v<remove_ref_cv(Functor)>)>
auto for_each(const std::tuple<Ts...> &t, Functor &&functor) noexcept(noexcept(tuple_for_eacher(t, std::declval<decltype(std::forward<Functor>(functor))>(), std::declval<std::make_index_sequence<std::tuple_size<std::tuple<Ts...>>::value>>())))
    -> const std::tuple<Ts...> &
{
...
template <bool b, class Tuple, class Functor, REQUIRES(is_callable_v<remove_ref_cv(Functor)>)>
...
template <class T, class Functor, REQUIRES(is_callable_v<remove_ref_cv(Functor)>)>
