...
#ifndef INCLUDED_SROOK_NUMERIC_ALGORITHM_ACCUMULATE_HPP
#define INCLUDED_SROOK_NUMERIC_ALGORITHM_ACCUMULATE_HPP
...
{
    return srook::numeric::algorithm::accumulate(std::begin(range), std::end(range), init);
}
...
{
    return srook::numeric::algorithm::accumulate(std::begin(range), std::end(range), init, op);
}
