...
#ifndef INCLUDED_SROOK_NUMERIC_ALGORITHM_INNER_PRODUCT_HPP
#define INCLUDED_SROOK_NUMERIC_ALGORITHM_INNER_PRODUCT_HPP
...
{
    return srook::numeric::algorithm::inner_product(std::begin(range), std::end(range), first, value);
}
...
{
    return srook::numeric::algorithm::inner_product(std::begin(range), std::end(range), first, value, op1, op2);
}
