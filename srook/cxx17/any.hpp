...
struct any{
	...
	std::remove_reference_t<srook::First_t<Ts...>> plus(Ts&&... args)const
	{
		return plus_core(
				static_cast<
					const std::remove_reference_t<First_t<Ts...>>
				>(**static_cast<Erasure<std::remove_reference_t<First_t<Ts...>>>*>(ptr_.get())),
				std::forward<Ts>(args)...
		);
	}
	...
	operator T()const{return get<std::remove_cv_t<std::remove_reference_t<T>>>();}

	template<class T>
	T& get()const noexcept{return **static_cast<Erasure<T>*>(ptr_.get());}
	template<class T>
	T& get(){return **static_cast<Erasure<T>*>(ptr_.get());}
	...
	template<class T>
	static T& get(const any& this_){return this_.get<T>();}
private:
	struct Erasure_core{
...
#ifdef POSSIBLE_TO_INCLUDE_BOOST_TYPEINDEX	
		const std::string pretty_name()const override{return boost::typeindex::type_id<T>().pretty_name();}
#endif
	...
	template<class Tail>
	constexpr Tail&& plus_core(Tail&& tail)const{return std::forward<Tail>(tail);}
	template<class Tail>
	constexpr Tail&& minus_core(Tail&& tail)const{return std::forward<Tail>(tail);}
	template<class Tail>
	constexpr Tail&& multiply_core(Tail&& tail)const{return std::forward<Tail>(tail);}
	template<class Tail>
	constexpr Tail&& division_core(Tail&& tail)const{return std::forward<Tail>(tail);}
