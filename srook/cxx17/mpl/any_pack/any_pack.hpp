...
struct any_pack {
    ...
    static constexpr std::conditional_t<!size(), std::nullopt_t, At_t<target, decltype(v)...>> at()
    {
	return detail::at_v<target, v...>;
    }
    ...
    template <template <class...> class Range, class Transformer = Nothing>
    static decltype(detail::transfer<Range, Transformer()(v)...>::value) range;
    template <template <class...> class ConstantRange, class Transformer = Nothing>
    static constexpr std::enable_if_t<is_invocable_v<Transformer, First_t<decltype(v)...>>, ConstantRange<std::decay_t<decltype(Transformer()(v))>...>> constant_range{Transformer()(v)...};
    ...
    template <std::size_t begin, std::size_t end, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = std::conditional_t<(begin < end), detail::Increment, detail::Decrement>>
    using for_to = detail::for_to_t<begin, end, Applyer, Crease, Parameter, any_pack<v...>, TypePack>;
    template <std::size_t begin, std::size_t end, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = std::conditional_t<(begin < end), detail::Increment, detail::Decrement>>
    ...
    template <std::size_t begin, class conditional_End, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = detail::Increment>
    using for_conditional = detail::for_conditional_t<begin, conditional_End, Applyer, Crease, Parameter, any_pack<v...>, TypePack>;
    template <std::size_t begin, class conditional_End, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = detail::Increment>
    using for_conditional_cut = detail::for_conditional_cut_t<begin, conditional_End, Applyer, Crease, Parameter, any_pack<v...>, TypePack>;
    template <std::size_t begin, class conditional_End, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = detail::Increment>
    using for_type_conditional = detail::for_type_conditional_t<begin, conditional_End, Applyer, Crease, Parameter, any_pack<v...>, TypePack>;
    template <std::size_t begin, class conditional_End, template <std::size_t, class, class, class> class Applyer, class Parameter = any_pack<>, class TypePack = pack<>, class Crease = detail::Increment>
