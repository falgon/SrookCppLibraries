...
#ifndef INCLUDED_SROOK_RANGE_PIPE_ALGORITHM_SET_UNION_HPP
#define INCLUDED_SROOK_RANGE_PIPE_ALGORITHM_SET_UNION_HPP
...
struct set_union_t{
	...
	explicit constexpr set_union_t(InputIterator first,InputIterator last,OutputIterator oiter)
	...
	>
	explicit constexpr set_union_t(Range&& r,OutputIterator oiter)
	...
	{
		return std::set_union(r.begin(),r.end(),std::move(first_),std::move(last_),std::move(oiter_));
	}
protected:
...
struct set_union_compare_t final:set_union_t<InputIterator,OutputIterator>{
	using set_union_t<InputIterator,OutputIterator>::set_union_t;

	template<
		REQUIRES(!has_iterator_v<InputIterator> and !has_iterator_v<OutputIterator> and is_callable_v<Compare>)
	>
	explicit constexpr 
	set_union_compare_t(InputIterator first,InputIterator last,OutputIterator oiter,Compare comp)
		:set_union_t<InputIterator,OutputIterator>::set_union_t(
	...
	>
	explicit constexpr
	set_union_compare_t(Range&& r,OutputIterator oiter,Compare comp)
		:set_union_t<InputIterator,OutputIterator>::set_union_t(r.begin(),r.end(),std::move(oiter)),
		comp_(std::move(comp))
	{}
...
>
set_union(Range&& r,OutputIterator oiter)
{
	return set_union_t<
...
>
constexpr set_union_compare_t<
	std::remove_reference_t<std::remove_cv_t<InputIterator>>,
	std::remove_reference_t<std::remove_cv_t<OutputIterator>>,
	std::remove_reference_t<std::remove_cv_t<Compare>>
>
set_union(InputIterator&& first,InputIterator&& last,OutputIterator oiter,Compare&& comp)
{
	return set_union_compare_t<
		std::remove_reference_t<std::remove_cv_t<InputIterator>>,
		std::remove_reference_t<std::remove_cv_t<OutputIterator>>,
		std::remove_reference_t<std::remove_cv_t<Compare>>
	>(
...
constexpr set_union_compare_t<
	...
	std::remove_reference_t<std::remove_cv_t<Compare>>
>
set_union(Range&& r,OutputIterator&& oiter,Compare&& comp)
{
	return set_union_compare_t<
		typename std::remove_reference_t<std::remove_cv_t<Range>>::const_iterator,
		std::remove_reference_t<std::remove_cv_t<OutputIterator>>,
