...
struct remove_copy_if_iterator{
	...
	template<
		class Range,
		REQUIRES(has_iterator_v<std::decay_t<Range>> or is_range_iterator_v<std::decay_t<Range>>)
	>
...
template<
	class Iterator,
	class Predicate,
	REQUIRES(!has_iterator_v<std::decay_t<Iterator>> and is_callable_v<std::decay_t<Predicate>>)
>
...
template<
	...
	class Predicate,
	REQUIRES(
			(has_iterator_v<std::decay_t<Range>> || is_range_iterator_v<std::decay_t<Range>>) and
		   	is_callable_v<std::decay_t<Predicate>>
	)
>
