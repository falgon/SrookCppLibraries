...
#if SROOK_HAS_THREADS && (SROOK_HAS_INCLUDE(<chrono>) || SROOK_HAS_INCLUDE(<boost/chrono>))
...
#		if SROOK_HAS_THREAD_API_PTHREAD
			...
			bool thread_id_equal(thread_id_type t1, thread_id_type t2) 
			SROOK_NOEXCEPT_TRUE														{ return pthread_equal(t1, t2) != 0; }
			bool thread_id_less(thread_id_type t1, thread_id_type t2)
			SROOK_NOEXCEPT_TRUE														{ return t1 < t2; }
...
#			endif
			...
			int tls_create(tls_key_type* key, void (*at_exit)(void*)) 				{ return pthread_key_create(key, at_exit);}
...
#			endif
				...
				SROOK_CPP03_AUTO(declval<nanoseconds>() + declval<seconds>(), nanoseconds) duration = 
...
#			endif
			...
			{
				const Begin_threadex_thunk_data* const data = 
					new
#			if !(SROOK_CPLUSPLUS >= SROOK_CPLUSPLUS11_CONSTANT)
...
#			endif
			...
			{
				return WaitForSingleObjectEx(*t, INFINITE, FALSE) == WAIT_FAILED ? 
					GetLastError : !CloseHandle(*t) ?
				   		GetLastError() : 0;
			}
...
#				if SROOK_HAS_INCLUDE(<chrono>)
					DEF_THREAD_SLEEP_FOR(std)
#				elif SROOK_HAS_INCLUDE(<boost/chrono>)
					DEF_THREAD_SLEEP_FOR(boost)
#				endif
#				undef DEF_THREAD_SLEEP_FOR
#			endif
