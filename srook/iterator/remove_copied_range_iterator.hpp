...
make_remove_copied_range_iterator(Iterator&& first,Iterator&& last,T&& t)
{
	return remove_copied_range_iterator<
		...
		std::remove_cv_t<std::remove_reference_t<decltype(remove_copied_skipper)>>
	>(std::forward<Iterator>(first),std::forward<Iterator>(last),std::forward<T>(t),std::move(remove_copied_skipper));
}
