...
make_take_while_iterator(Pred&& pred, Iter first, Iter last) { return take_while_iterator<SROOK_DEDUCED_TYPENAME decay<Pred>::type, Iter>(first, last, srook::forward<Pred>(pred)); }
