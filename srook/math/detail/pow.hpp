...
template <class T>
inline constexpr T pow2(const T &x) SROOK_NOEXCEPT(x *x)
{
    return x * x;
}

template <class T>
inline constexpr T pow3(const T &x) SROOK_NOEXCEPT(x *x *x)
{
    return x * x * x;
}

template <class T>
inline constexpr T pow4(const T &x) SROOK_NOEXCEPT(x *x *x *x)
{
    return x * x * x * x;
}

template <class T>
inline constexpr T pow5(const T &x) SROOK_NOEXCEPT(x *x *x *x *x)
{
    return x * x * x * x * x;
}
...
inline constexpr T pow_n(const T &x, UIntType n) SROOK_NOEXCEPT(n == 0 ? T(1) : pow_n_impl(x, n))
{
...
inline constexpr T pow_n(const T &x, SIntType n) SROOK_NOEXCEPT()
{
