...
#define VARIADIC_REQUIRES(...)\
   	typename srook::enable_if<static_cast<bool>((__VA_ARGS__ + ...)), SROOK_NULLPTR_T>::type = SROOK_NULLPTR
#define REQUIRES(...)\
   	typename srook::enable_if<__VA_ARGS__, SROOK_NULLPTR_T>::type = SROOK_NULLPTR
#define SROOK_VARIADIC_REQUIRES(...) VARIADIC_REQUIRES(__VA_ARGS__)
#define SROOK_REQUIRES(...) REQUIRES(__VA_ARGS__)
#else
#define REQUIRES(...)\
	typename srook::enable_if<__VA_ARGS__, SROOK_NULLPTR_T>::type
#endif
#endif
