...
struct brainfk_syntax_llvm_compiler_core : brainfk_syntax_core<String, FileStream, BYTE, BUFFER_SIZE> {
	...
	llvm_wrap->allocate(BUFFER_SIZE);
	{
		...
		switch (this->tokenizer[i]) {
			...
			if (--iter < std::begin(buff))
			    return (this->fl = NULLOPT);
...
private:
	...
	explicit llvm_brainfk_wrapper()
	    : context_{}, builder_{context_}, module_{std::make_unique<llvm::Module>("top", context_)}, main_function_{llvm::Function::Create(llvm::FunctionType::get(llvm::Type::getInt32Ty(context_), false), llvm::Function::ExternalLinkage, "main", module_.get())}, data_{nullptr}, ptr_{nullptr}
	{
	    using namespace llvm;
