...
SROOK_ST_ASSERT(
    (srook::type_traits::detail::Land<
        srook::is_same<
            decltype(st1),
            srook::thread_safe::container::stack<int, std::deque<int>, srook::thread_safe::container::behavior::wait_pushed> > >::value));
...
void thread14()
{
    ...
    for (std::size_t i = 0; i < st1.size(); ++i) {
        guard_print(st1.pop().value_or(-1)); // 42
    }

    if (!st2.empty()) {
        for (std::size_t i = 0; i < st2.size(); ++i) {
            guard_print(st2.pop().value_or(-1)); // If this statement is executed, the value is always 42
        }
    }
}
...
void exec1()
{
    srook::thread th1(thread13), th2(thread14);
    th1.join();
    th2.join();
}
